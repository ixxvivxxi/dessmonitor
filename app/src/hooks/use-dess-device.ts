...
function parseDeviceData(res: BackendLatestResponse): DeviceData {
  ...
  const gf = findVal(gd, ['gd_ac_input_frequency', 'gd_grid_frequency', 'gd_rid_frequency']);
  const pvv = findVal(pv, ['pv_input_voltage', 'pv_voltage1']);
  const pvp = findVal(pv, ['pv_output_power', 'pv_input_power', 'pv_power1']);
  ...
  const lp = findVal(bc, ['bc_output_apparent_power', 'bc_load_active_power', 'bc_output_power']);
  ...
  const parseNum = (s: string | null) => (s != null ? Number.parseFloat(s) : null);
  ...
  return {
    ...
    battery: {
      ...
      capacity: parseNum(btc) ?? (btc != null ? Number.parseInt(btc, 10) : null),
